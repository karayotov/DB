namespace P02_DatabaseFirst
{
    ...
    public class Engine
    {
        public void Run()
        {
            //this.EmployeesFullInformation();                                                                 //03. Employees Full Information
            //this.EmployeeWithSalaryOver(50_000);                                                             //04. Employees with Salary Over 50 000
            //this.EmployeeFromDevelopment("Research and Development");                                        //05. Employees from Research and Development
            //this.AddingNewAddressToEmployee("Vitoshka 15", 4, "Nakov");                                      //06. Adding a New Address and Updating Employee
            //this.EmployeesProjects();                                                                        //07. Employees and Projects
            //this.AddressByTown();                                                                            //08. Addresses by Town
            //this.EmployeeById(147);                                                                          //09. Employee 147
            ...
            //this.IncreaseSalaries(12, "Engineering", "Tool Design", "Marketing", "Information Services");    //12. Increase Salaries                              //12. Increase Salaries
            //this.FindEmployeesWithFirstNameStartingWith("Sa");                                               //13. Find Employees by First Name Starting With "Sa"
        ...
        private void RemoveTowns(params string[] names)
        {
            using (var context = new SoftUniContext())
            {
                foreach (var name in names)
                {
                    ...
                    string addressPloralisation = addressesCount == 1
                        ? "address"
                        : "addresses";

                    Console.WriteLine($"{addressesCount} {addressPloralisation} in {name} was deleted");
                }
            }
        }
        ...
        private void FindEmployeesWithFirstNameStartingWith(string startingWith)
        {
            using (var context = new SoftUniContext())
            {
        ...
        private void EmployeesProjects()
        {
            using (var context = new SoftUniContext())
            {
                ...
                {
                    Console.WriteLine($"{employee.Employee} - Manager: {employee.Manager}");
        ...
        private void EmployeeFromDevelopment(string department)
        {
            using (var context = new SoftUniContext())
            {
                Console.WriteLine(string.Join(Environment.NewLine, context.Employees
        ...
        private void EmployeeWithSalaryOver(int salary)
        {
            using (var context = new SoftUniContext())
            {
                Console.WriteLine(string.Join(Environment.NewLine, context.Employees
