...
namespace exercises_01
{
    public class StartUp
    {
        public static void Main(string[] args)
        {
            ...
            string tableTowns = "CREATE TABLE Towns(Id INT PRIMARY KEY IDENTITY, Name (50), CountryCode INT FOREIGN KEY REFERENCES Countries(Id))";
            ...
            using (SqlConnection connection = new SqlConnection(Configuration.ConnectionString))
            {
                ...
                ExecNonQuery(databaseSql, connection);

                connection.ChangeDatabase("MinionsDB");

                ExecNonQuery(tableCountries, connection);

                ExecNonQuery(tableTowns, connection);

                ExecNonQuery(tableMinions, connection);

                ExecNonQuery(tableEvilnessFactors, connection);

                ExecNonQuery(tableVillains, connection);

                ExecNonQuery(tableMinionsVillains, connection);

                ExecNonQuery(insertCountries, connection);

                ExecNonQuery(insertTowns, connection);

                ExecNonQuery(insertMinions, connection);

                ExecNonQuery(insertEvilness, connection);

                ExecNonQuery(insertVillains, connection);

                ExecNonQuery(insertMinionsVillains, connection);

                connection.Close();
            }
        }

        private static void ExecNonQuery(string databaseSql, SqlConnection connection)
        {
